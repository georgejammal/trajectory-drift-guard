...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--counting-datasets",
        default=None,
        help="Optional comma-separated name=path mappings for counting, e.g. 'how_many=data/how_many_validation,tallyqa=data/tallyqa_validation'.",
    )
...
def main() -> None:
    ...
    args = parse_args()
    if args.counting_datasets:
        mappings = {}
        for item in args.counting_datasets.split(","):
            name, separator, path = item.partition("=")
            if not separator or not name or not path:
                raise ValueError("--counting-datasets entries must use name=path.")
            mappings[name] = path
        args.counting_datasets = mappings
